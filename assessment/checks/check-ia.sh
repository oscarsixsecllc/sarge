...
if ! platform_supports login_defs_value; then
  ...
  _IA5_RC=$?
  if [[ $_IA5_RC -eq 127 ]]; then
    skipx "IA-5-pass-max-days" "IA-5: Password aging is delegated to MDM on this ${SARGE_OS_DESCRIPTION}. Review MDM compliance reports for NIST IA-5 coverage"
  else
...
if ! platform_supports pwquality_config_path; then
  ...
  _IA5_PW_RC=$?
  if [[ $_IA5_PW_RC -eq 127 ]]; then
    skipx "IA-5-pwquality-missing" "IA-5: Password complexity is delegated to MDM on this ${SARGE_OS_DESCRIPTION}. Review MDM compliance reports"
  elif [[ -f "$PWQUAL" ]] || [[ "$SARGE_OS" == "macos" ]]; then
    # On macOS the "path" is a sentinel (pwpolicy backing store, not a real file).
    # Proceed to value checks regardless.
...
if ! platform_supports pam_auth_path; then
  ...
  _IA2_RC=$?
  if [[ $_IA2_RC -eq 127 ]]; then
    skipx "IA-2-faillock-not-configured" "IA-2: Account lockout is delegated to MDM on this ${SARGE_OS_DESCRIPTION}. Review MDM compliance reports"
  elif [[ -f "$PAM_AUTH" ]] || [[ "$SARGE_OS" == "macos" ]]; then
    # On macOS the "path" is a sentinel; proceed to value checks.
    if platform pam_faillock_configured; then
      passx "IA-2-faillock-not-configured" "IA-2: Account lockout policy is configured"
      FAILLOCK_CONF=$(platform faillock_config_path)
      if [[ -f "$FAILLOCK_CONF" ]] || [[ "$SARGE_OS" == "macos" ]]; then
        DENY=$(platform faillock_value deny)
        UNLOCK_TIME=$(platform faillock_value unlock_time)
        if [[ -n "$DENY" && "$DENY" -le 5 ]]; then
          passx "IA-2-faillock-deny" "IA-2: Lockout deny threshold is $DENY (<=5 attempts)"
        else
          warnx "IA-2-faillock-deny" "IA-2: Lockout deny is ${DENY:-unset}. Recommend 5 or fewer attempts"
        fi
        if [[ -n "$UNLOCK_TIME" && "$UNLOCK_TIME" -ge 1800 ]]; then
          passx "IA-2-faillock-unlock-time" "IA-2: Lockout unlock_time is $UNLOCK_TIME seconds (>=30 min)"
        else
          warnx "IA-2-faillock-unlock-time" "IA-2: Lockout unlock_time is ${UNLOCK_TIME:-unset}. Recommend 1800 (30 min)"
        fi
      else
        warnx "IA-2-faillock-conf-missing" "IA-2: pam_faillock referenced but $FAILLOCK_CONF not found"
      fi
    else
      failx "IA-2-faillock-not-configured" "IA-2: Account lockout policy not configured. No lockout enforcement detected"
    fi
  fi
fi
