...
> **macOS coverage.** The numbers above are Ubuntu 24.04 only. On macOS, gap analysis and drift detection now run natively. Password policy (IA-5/IA-2) probes `pwpolicy -getaccountpolicies`, pending updates (CM-6/SI-2) probe `softwareupdate --list --no-scan`, and drift snapshots include `pending_updates` fields. Controls without a macOS-native equivalent (auditd, clamav, fail2ban) emit SKIP with a platform-aware rationale. MDM-managed Macs get clean SKIPs with an MDM-delegation rationale rather than misleading FAILs. Native macOS hardening modules (firewall, SSH, logging policy) are landing per-PR.
